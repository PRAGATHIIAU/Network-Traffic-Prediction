...
#NB15
network_traffic_data = join(data_path, "NetworkTrafficNB15.csv")
network_graph_file = join(data_path, "weighted_adjacency_matrix.pkl")
network_train_csv = join(data_path, "network_train.csv")
network_test_csv = join(data_path, "network_test_csv.csv")
gcn_lstm_csv = join(data_path, "training_log_ruk.csv")
gcn_lstm_knn_csv = join(data_path, "training_log_knn_ruk.csv")
testX_npy = join(data_path, "testY_ruk.npy")
predictions_npy = join(data_path, "predictions_ruk.npy")
protocol_and_attack_csv = join(data_path, "protocol_and_attack.csv")
# Pre-processing
min_number_of_observations = 1   # Drop stations having too few observations
